    ...
    ILogger<OrukServiceSearchTool> logger)
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (targets.Count == 0)
            return JsonSerializer.Serialize(new { error = $"Feed URL not found in configured feeds: {feedUrl}" });

        var maxTotal = options.Value.MaxResultsPerQuery;
        var results = new List<ServiceWithOrigin>();

        // Fan out search across target feeds in parallel
        ...
        var tasks = targets.Select(async targetFeedUrl =>
        {
            try
            {
                ...
                if (termIds.Count > 0)
                    ...
                    logger.LogInformation(
                        "No taxonomy match for '{Query}' in feed {FeedUrl} — client-side keyword filter will be applied.",
                        query, targetFeedUrl);
    ...
    private List<Uri> ResolveFeedTargets(string? feedUrl)
    {
        ...
        // Match by normalised URL (strip trailing slash / /services suffix for comparison)
